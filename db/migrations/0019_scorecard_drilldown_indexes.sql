...
-- (item_code, invoice_date). Major and minor drill-down pages filter by
...
CREATE INDEX IF NOT EXISTS idx_csf_item_date
  ON public.customer_scorecard_fact (item_code, invoice_date DESC NULLS LAST)
...
CREATE INDEX IF NOT EXISTS idx_csf_branch_item_date
  ON public.customer_scorecard_fact (branch_id, item_code, invoice_date DESC NULLS LAST)
  WHERE is_deleted = false AND item_code IS NOT NULL;
