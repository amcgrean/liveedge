...
**Phase 3 — Daily check ingest (code landed 2026-05-21, MIGRATION NOT YET APPLIED):** Original brief at `docs/agent-prompts/hubbell-daily-check-ingest-2026-05-20.md`, resolved decisions in `docs/agent-prompts/hubbell-daily-check-ingest-addendum-2026-05-20.md`. **Apply migration `0026_hubbell_checks.sql` manually in the Supabase SQL editor before deploying — until then the new code targets tables that don't exist yet.** Phase 3a/b shipped:
...
11. **Hubbell daily check ingest** (Phase 3a/b CODE LANDED 2026-05-21 — MIGRATION NOT YET APPLIED): Server-side code (migration `0026_hubbell_checks.sql`, `POST /api/admin/hubbell/checks/upload`, rewritten `/payments/import`, rollup helpers in `src/lib/hubbell/payment-rollup.ts`, canonical hash in `src/lib/hubbell/check-hash.ts`) all shipped. **Next action: apply migration 0026 manually in Supabase SQL editor.** It backfills `hubbell_document_payments` (4,879 rows) into the new tables, refreshes rollups, drops the old table, and contains a `DO $$ RAISE EXCEPTION` guard that fails the migration if backfill row count drifts. After apply, deploy the code. Then Phase 3c (Pi `hubbell_daily_checks.py` scraper) and Phase 3d (`GET /api/hubbell/docs` + `/checks` read endpoints for the monthly recon) remain. See the Hubbell PO/WO Daily Ingest section above + `docs/agent-prompts/hubbell-daily-check-ingest-addendum-2026-05-20.md`.
