...
**Proposed Phase 3 — Daily check ingest (handoff 2026-05-20, NOT YET STARTED):** Original brief at `docs/agent-prompts/hubbell-daily-check-ingest-2026-05-20.md`, **resolved decisions in `docs/agent-prompts/hubbell-daily-check-ingest-addendum-2026-05-20.md` — read both before building**. Goal: fold paid-check data into the daily Pi run so month-end recon (currently a 90-min PC portal scrape of ~4,600 paid-check PDFs two days before EOM) collapses to a report-gen step. Pieces:

1. **New tables + retire old payments table** (migration `0026_hubbell_checks.sql`, schema-portable for an eventual `bids → hubbell` schema move):
   - `bids.hubbell_checks` — keys: `(payer_entity, check_number)` UNIQUE (NOT `check_number` alone — check numbers aren't guaranteed unique across HUBB1000/1200/1400/1700 entities), `source_hash` UNIQUE for idempotent re-POST. Columns: `check_date`, `total_amount`, `payment_count`, `payer_entity`, `source_run_id`, `first_seen_at`, `last_seen_at`.
   - `bids.hubbell_check_lines` — one row per check line. FK `check_id` → `hubbell_checks(id) ON DELETE CASCADE`. Columns: `doc_type` (`'po'`|`'wo'`|`'inv'`), `doc_number`, `invoice_date`, `payment_amount` (numeric, can be negative for credits), `gross_amount`, `memo`, `line_seq`. Index `(doc_type, doc_number)` for joining to `hubbell_documents`. **No `(memo)` index** — no planned query path uses it.
   - `bids.hubbell_payer_entity_lookup` — `dev_code → payer_entity` mapping (~47 rows from the Pi's dev list, populated manually by the user before this migration runs).
   - **DROP `hubbell_document_payments` in the SAME migration** after backfilling its 4,879 rows into `hubbell_check_lines` and rewriting the `hubbell_documents` rollup trigger to read from `hubbell_check_lines`. `check_lines` becomes the canonical source of payment facts — no two-tables-driving-the-same-rollup drift.
2. **`POST /api/admin/hubbell/checks/upload`** — Bearer `HUBBELL_UPLOAD_TOKEN`. Body `{payer_entity, check_number, source_run_id, check_date, lines:[...]}`. **Canonical `source_hash`**: sort lines by `(doc_type, doc_number, line_seq)`, convert all $ to integer cents, fixed key order in `JSON.stringify`, then sha256 — not raw `JSON.stringify` (float drift + key-order non-determinism would churn the hash). **Update semantics: wipe-and-replace inside a transaction** when source_hash differs (not add-only) — handles edits/deletions cleanly, preserves the check's UUID. Response statuses: `'inserted'` | `'unchanged'` | `'replaced'`. See addendum §3-§4 for the exact resolver and transaction code.
3. **Pi-side scraper** — new `hubbell_daily_checks.py` invoked from the same systemd service after the PO/WO scrape. Hits `https://hub.ihmsweb.com/cgi-bin/ihmsweb.exe?pgm=marwbvo`, reuses `parse_checks_from_marwbvo()` + per-check detail parser from `hubbell_checks_to_pdfs_po_and_wo.py` / `export_portal_invoice_table_v8_full.py`. Default `--max_checks 6` (≈1 month); `--max_checks 0` for one-time historical backfill. Shares ECI session + dev-list cache with the PO/WO scrape. `payer_entity` derived server-side from the dominant `dev_code` on the check's lines via the lookup table; `'UNKNOWN'` when ambiguous (don't block ingest, flag in admin UI).
4. **Read endpoints** (later, once data is flowing): `GET /api/hubbell/docs?since=YYYY-MM-DD&cursor=...&limit=200` and `GET /api/hubbell/checks?since=...&cursor=...&limit=200` — both Bearer-protected, **cursor-based pagination** (cursor encodes `(check_date, id)`; default 200, max 1000) since data is mostly append-only and offset pagination would skip rows on concurrent inserts. **Two-path SO resolver** for check lines: `po`/`wo` → `hubbell_documents → hubbell_document_sos`; `inv` → `agility_so_header` directly via `ref_num`. Endpoint returns identical shape (`document_id?`, `attached_so_ids[]`, `resolution_path`) so the consumer doesn't branch on doc_type. PC's `hubbell_reconciliation_v1.py` gets a `--mode liveedge` flag; the existing scrape path stays as fallback. Once a few months validate, the 90-min portal-scrape step in `hubbell_run.py` drops.
5. **Schema-move sequencing**: `bids → hubbell` rename **deferred until after Phase 3d** (PC scripts retired). Otherwise we do the Drizzle codebase rewrite twice. Compatibility view trick doesn't help Drizzle (targets a specific schema at codegen). Plan: Phase 3e is `ALTER SCHEMA` + Drizzle codegen update in one focused PR with no external consumers left.
...
11. **Hubbell daily check ingest** (proposed 2026-05-20, NOT STARTED): Brief at `docs/agent-prompts/hubbell-daily-check-ingest-2026-05-20.md` + **resolved decisions at `docs/agent-prompts/hubbell-daily-check-ingest-addendum-2026-05-20.md`** — read both. Migration 0026 adds `bids.hubbell_checks` + `bids.hubbell_check_lines` + `bids.hubbell_payer_entity_lookup`, **drops `hubbell_document_payments` in the same migration** (canonical source moves to `check_lines`), and rewrites the `hubbell_documents` rollup to read from the new tables. Endpoint `POST /api/admin/hubbell/checks/upload` uses canonical `source_hash` (cents-int + fixed key order) and wipe-and-replace tx semantics. Pi sibling script `hubbell_daily_checks.py` scrapes Payment History (`marwbvo`) daily after the PO/WO run. Read endpoints `GET /api/hubbell/docs` + `GET /api/hubbell/checks` are cursor-paginated with a two-path `po|wo|inv` SO resolver. Schema move from `bids → hubbell` deferred until after PC scripts retire (Phase 3d). Outstanding pre-work: user supplies `dev_code → payer_entity` mapping (~47 rows).
