...
1. **New tables + retire old payments table** (migration `0026_hubbell_checks.sql`, schema-portable for an eventual `bids → hubbell` schema move):
   - `bids.hubbell_checks` — keys: `check_number` UNIQUE (HUBB1000/1200/1400/1700 are Beisser-side AR accounts for work type, NOT separate Hubbell payer entities — all checks come from one Hubbell payer via `vendornumber=000658`, sequential numbering confirms one issuance stream), `source_hash` UNIQUE for idempotent re-POST. Columns: `check_date`, `total_amount`, `payment_count`, `source_run_id`, `first_seen_at`, `last_seen_at`. **No `payer_entity` column.**
...
11. **Hubbell daily check ingest** (proposed 2026-05-20, NOT STARTED): Brief at `docs/agent-prompts/hubbell-daily-check-ingest-2026-05-20.md` + **resolved decisions at `docs/agent-prompts/hubbell-daily-check-ingest-addendum-2026-05-20.md`** — read both. Migration 0026 adds `bids.hubbell_checks` (key: `check_number` UNIQUE) + `bids.hubbell_check_lines`, **drops `hubbell_document_payments` in the same migration** (canonical source moves to `check_lines`), and rewrites the `hubbell_documents` rollup to read from the new tables. Endpoint `POST /api/admin/hubbell/checks/upload` uses canonical `source_hash` (cents-int + fixed key order) and wipe-and-replace tx semantics. Pi sibling script `hubbell_daily_checks.py` scrapes Payment History (`marwbvo`) daily after the PO/WO run. Read endpoints `GET /api/hubbell/docs` + `GET /api/hubbell/checks` are cursor-paginated with a two-path `po|wo|inv` SO resolver. Schema move from `bids → hubbell` deferred until after PC scripts retire (Phase 3d). **No outstanding pre-work** — ship 0026 when ready.
