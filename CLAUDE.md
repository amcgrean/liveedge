...
**REFRESH … CONCURRENTLY needs a PLAIN unique index (0037 fix, 2026-06-01):** `REFRESH MATERIALIZED VIEW CONCURRENTLY` requires a unique index of **plain columns only** — no expressions, no WHERE clause. `rollup_product_day` / `rollup_saletype_day` were given **expression** unique indexes (`(COALESCE(branch_id,''))`, …) — and `saletype_day` carries `NULL customer_id` by design (its multi-scope UNION) — so their concurrent nightly refresh failed instantly every night from 0036 until 0037, leaving the MVs frozen at create time (caught by the sync-health alert's first real run). Fix (`0037_fix_rollup_refresh_concurrently.sql`): refresh those two **non-concurrently** (no unique-index requirement; the ACCESS EXCLUSIVE lock is fine at 04:15 Central). `rollup_customer_day` keeps CONCURRENTLY (plain `(branch_id, customer_id, d)` index). **Rule for any new rollup: if it needs CONCURRENTLY refresh, give it a plain-column, NULL-free, non-partial unique index; multi-scope UNION rollups can't satisfy that without sentinel values — use non-concurrent refresh for those.** **Applied 2026-06-01** — both jobs verified non-concurrent via `cron.job`; they refresh on schedule again (re-run `GET /api/cron/sync-health-alert?test=1` to confirm `healthy:true`).
...
**Slice 2b+ / fact-gap closeout (2026-05-31, PR #471):** completeness sweep against the original gameplan found three more functions still scanning the 6.4 GB fact; rewired:
- `_fetchBranchSummaries` → `rollup_customer_day`. `customer_count` is `COUNT(DISTINCT customer_id)` and stays **exact** because `customer_id` is in the rollup grain (validated 425 = 425, Δ0).
- `_fetchProductScorecardMajors` / `_fetchProductScorecardMinors` (the `/scorecard/product` page) → additive (`sales`/`gp`/`qty_shipped`) from `rollup_product_day`; exact distinct SO/credit counts via `fetchProductScorecardCounts()` (bounded live grouped by product code) — same additive/distinct split as the KPI paths. `buildProdRollupSelect()` is the rollup analog of `buildProdSelect()` (the latter stays for the live item-level function).

**Pi-sync staleness alerting (2026-05-31, PR #471):** `computeSyncHealth()` extracted to `src/lib/admin/sync-health.ts` (shared by the dashboard route + the alert cron) and expanded to cover **all three** rollups (was customer-only). New `GET /api/cron/sync-health-alert` (daily 10:00 UTC, after the rollup refreshes) emails `SYNC_HEALTH_ALERT_TO` when the fact sync or any rollup is stale/failed. One daily run = at most one alert/day while stale, so **no alert-state table needed**. Always `console.warn`s the issue list regardless of email config. `GET …?test=1` (auth-gated) forces a sample alert send even when healthy, to verify delivery/rendering on demand. Env: `SYNC_HEALTH_ALERT_TO` (comma/semicolon list), `SYNC_ALERTS_EMAIL_FROM` (default `LiveEdge Ops <noreply@app.beisser.cloud>`), `SYNC_ALERTS_CONSOLE=true` (dev: log instead of send). Reuses `RESEND_API_KEY` + `CRON_SECRET`.

**Tier 1 status: COMPLETE for the fact-contention objective.** Every heavy `customer_scorecard_fact` scan now reads a rollup; what stays live is intentional (single-customer point lookups, distinct-count-exact grouped queries, `_searchCustomers`, `_fetchDaysToPay`, `_fetchProductOrders`, item-level drill, rep-scoped joins). **Slice 2c (`rollup_vendor_day`) deliberately NOT built — measured decision:** the vendor source (`agility_receiving_*` + `agility_po_*`) totals ~600 MB and is well-indexed, vs the 6.4 GB fact; it was never the 2026-05-28 contention path. Build it only if vendor scorecard pages measurably slow. The other Tier-1 tail is hardware/ops: the Pi 5 + SSD migration and a written recovery runbook (owner/Pi-agent, not LiveEdge code).

**Tier 2 — operational resilience: COMPLETE.** CI gates `tsc`+`lint` (PR #479, `.github/workflows/tests.yml`), unit tests on the catastrophic-if-wrong paths (PR #480 — `agility-api`, `takeoff/calculations`, `hubbell/jobsite-reconciler`+`po-number-parser`, `log`, on top of the existing access-control/capability/hubbell-hash tests), and Sentry + structured logger `src/lib/log.ts` (PR #483; env in the Observability section). Handoff that drove it: `docs/agent-prompts/tier-2-operational-resilience-2026-05-31.md`.

**Next: Tier 3 — hardening & maintainability** (OTP rate-limiting, zod at route boundaries, TanStack Query for client fetches, opportunistic god-file splits). Self-contained handoff at `docs/agent-prompts/tier-3-hardening-2026-06-01.md`.
...
18. **System-wide UOM display label remediation** (2026-05-31): `agility_so_lines.price_uom_ptr` is a raw FK into Agility's UOM master table. There's NO `agility_uoms` mirror in Supabase today — only `agility_items.display_uom` (the item's default display UOM, e.g. "Each" / "BF") is available. Mobile customer-sheet now joins `agility_items.display_uom` and falls back to suppressing pointers that look numeric. Same bug almost certainly exists on the web: anywhere a query selects `price_uom_ptr` (or stocking-pointer / weight-pointer FKs) and renders it raw, drivers/sales see "197" instead of "BF". **Owner: Pi sync agent first** — mirror the Agility UOM master into a new `public.agility_uoms` table (`code`, `display`, `factor`, `is_deleted`). Once that lands, do a repo-wide sweep: `git grep -nE 'price_uom_ptr|stocking_uom|picking_uom'` and swap to `LEFT JOIN agility_uoms u ON u.code = sol.price_uom_ptr` returning `u.display`. Until then, the lines route's `display_uom` fallback is the working pattern; mirror it anywhere similar surfaces.
19. **Pricing capability gate audit** (2026-05-31): added `CAPABILITIES.PRICING_VIEW = 'pricing.view'` granted by default to sales / management / supervisor / ops / estimator / estimating / purchasing / admin. NOT granted to driver / warehouse / dispatch / receiving_yard / hubbell / designer / viewer. `/api/dispatch/orders/[so]/lines` strips price / extended_price / unshipped_extended_price server-side when caller lacks it and returns `pricing_visible: boolean` so the client can hide the entire $ column (mobile customer sheet does). **Sweep needed:** any other API route that returns prices, totals, or margin-adjacent fields should mirror this pattern. Candidates: `/api/sales/orders/[so_number]/route.ts`, `/api/warehouse/orders/[so_number]/route.ts`, `/api/dispatch/orders/[so_number]/timeline/route.ts`, sales-hub aggregates, scorecard responses. Drivers shouldn't see ANY $ figures in the mobile app or anywhere reachable from a Bearer token. Web users are typically gated by route entirely, but mixed-role routes (any endpoint a driver could hit) need the same strip-on-the-server approach.
20. ~~**Suggested Buys — LMC1000 ship-from grouping is collapsing**~~ — FIXED (2026-06-01, uncommitted local diff). Purchasing manager flagged at review prep that `/purchasing/suggested-buys` was showing 177 red items under a single "Top Notch Distributors - LMC" (LMC1000) group spanning Dexter, LJ Smith, Paslode, Schlage, Top Notch ship-froms. **Diagnosis** (Supabase MCP, query in commit message): the engine SQL correctly resolves DIFFERENT ship-froms per item — `dexsev619pass` → seq 11 (Top Notch), `ljpop506038` → seq 2 (Novo), `schacc622key` → seq 11 (Top Notch), `ljpop4075` → seq 2 (Novo). The bug was on the consumer side: both `buildSupplierRollup` in `src/lib/purchasing/replenishment.ts` AND `SuggestedBuysClient.tsx` were keying on `r.supplierCode` ALONE, so all LMC1000 rows collapsed into the first-seen bucket regardless of their actual ship-from. **Fix:**
    - `SupplierRollup` interface gained a `shipFromSeq: number | null` field; `buildSupplierRollup` keys on `${supplierCode}::${shipFromSeq ?? 0}`. Additive type change, no breaking changes for `/api/purchasing/replenishment` consumers.
    - `SuggestedBuysClient.tsx`: grouping map keyed on `${r.supplierCode}::${r.shipFromSeq ?? 0}`, and the `key` prop on each rendered group uses the same composite so React doesn't reuse DOM across distinct ship-froms.
    - `OutagesClient.tsx` already got the LMC1000 namespacing on the vendor scorecard link in the same session — clicking through now routes to the correct ship-from scorecard instead of seq 1.
    **Rule for future supplier UI work**: anywhere you bucket, key, or link by `supplierCode`, also consider `shipFromSeq`. LMC1000 is the canonical multi-ship-from supplier. The vendor scorecard's `buildVendorRouteKey` helper (in `src/lib/scorecard/product-drill-queries.ts`) is the canonical link-building pattern — duplicate inline (with comment pointing back) rather than importing it into client code since that module pulls postgres into the bundle.
    **Did NOT** refactor `buildVendorRouteKey` into a client-safe `src/lib/scorecard/vendor-route-key.ts` — there are now three callers (the two server modules + the inline copy in OutagesClient) but the existing two-copy "keep in sync" smell was already known and a third inline isn't worth the extra abstraction.
    **Manager-meeting context**: purchasing manager loves the tool overall, called out specifically that he's "overwhelmed" by the volume of red items — partly the genuine signal, partly the `qty_on_hand` data-quality issue documented in the Buyer Workspace section, partly this grouping bug inflating perceived noise per supplier. With the fix in, the LMC1000 bucket should now split into ~5 separate groups (Top Notch, Novo, Banner, Linc Systems, Simpson Strong-Tie) — which is what the buyer actually wants per-PO. Demo script + cheat sheet for the 2026-06-02 review at `docs/purchasing-manager-review-2026-05-28.md`.
...
## Mobile App — Driver POD + Sales (2026-05-28 → 2026-05-31) — PHASES 1–5 LIVE + SO LOOKUP + SALES

Expo SDK 54 React Native app at `mobile-app/`, intentionally independent from the Next.js web app. Two role-gated experiences in one binary: driver POD + sales lookup/quote/order. PRs #445, #454, #456 (Phase 5), #457 (SO lookup), #469 (Agility shipment), #473 (sales), #475 (numpad + line items), #477 (pricing gate + UOM) all merged.

**Live in prod against `app.beisser.cloud`:**
- **Auth:** OTP via `POST /api/auth/send-otp` → `POST /api/auth/mobile/verify-otp` returns a JWT signed with the same `AUTH_SECRET` as NextAuth. Mobile-only counterpart added to the route-guards `public` allowlist.
- **`src/lib/mobile-auth.ts`** ships `signMobileToken` / `verifyMobileToken` / `getMobileSession` / `requireSessionOrMobile`. The combined guard accepts NextAuth cookie OR Bearer JWT and is in the `guardPatterns` allowlist.
- **Dispatch routes:** `GET /api/dispatch/routes?include=stops`, `POST /orders/[so]/deliver`, `POST /orders/[so]/pod`, `POST /orders/[so]/pod/upload-url` (R2 presigned PUT), `GET /orders/[so]` + `POST /claim` (SO lookup + ad-hoc per-user-per-day stops), `GET /orders/[so]/lines` (with pricing gate + UOM JOIN).
- **Driver POD flow:** assigned stops list → delivery details → camera (≥2 photos, persisted to `documentDirectory/pod-photos/`) → two-phase sync (presigned R2 PUTs + `/deliver` POST with `photo_keys[]`). Outbox is resumable per-photo via `photoUploads[]`. Synced rows are removed + photo files deleted.
- **SO lookup flow:** magnifying-glass icon on `AppStatusBar` → custom 3×4 numpad screen (no OS keyboard — avoids a freeze where the keyboard stayed up over `router.replace`'d screens) → looks up via `/orders/[so]` → result card shows pill (OPEN/STAGED/IN TRANSIT/DELIVERED/INVOICED via `describeAssignment`) + "Staged to <driver> · ship M/D/YYYY" line + customer/address/branch/PO/ref/line_count. If unassigned, "Take this stop" claim button creates ad-hoc dispatch stop server-side. Claim required before POD actions enable.
- **Customer sheet** at `/(app)/[soNumber]/customer` fetches real line items from `/orders/[so]/lines` and renders qty + UOM ("Each"/"BF") + desc + item_code + optional $ext + Subtotal.
- **Sales experience** (PR #473, role-gated via `RoleContext`): tabs (Home, Orders, Customers, Items, Profile), customer/item/order detail screens, new order + new quote flows. Single-role users land directly; dual-role users hit `role-switch`.

**Capability gates:**
- `dispatch.view` — driver route + lookup + claim + deliver (driver role grants by default)
- `sales.view` — sales experience + SO lookup (`/orders/[so]` accepts either)
- `pricing.view` — server-strips `price`/`extended_price` to null when missing; returns `pricing_visible: boolean`. Default grants: admin/management/sales/supervisor/ops/estimator/estimating/purchasing. NOT granted to driver/dispatch/warehouse/receiving_yard/designer/hubbell/viewer. **Server-side strip — bytes never reach the device.** Web users typically gated by route entirely, but mixed-role endpoints (any Bearer-reachable route) need the same approach.

**Offline outbox architecture — preserve in any new flow:**
- `src/storage/{outbox,sync,photoFS}.ts` are React-free — never import context. Token plumbing goes through `src/api/authToken.ts` (AuthContext writes, axios interceptor + sync engine read).
- `OutboxItem.photoUploads?: { uri, remoteKey?, uploaded }[]` enables resumable retry — a crash mid-upload doesn't cost already-uploaded photos.
- AsyncStorage outbox, 5-attempt backoff `[1s, 5s, 30s, 60s, 5m]`. Sync triggers: NetInfo offline→online, outbox enqueue, 30s heartbeat.
- Photo cleanup runs only after the deliver POST returns 2xx, not after the photo PUTs.
- `useStopOrLookup` hook merges `useDriverRoute()` (assigned stops) with on-demand `/orders/[so]` fetch — screens consume one DriverStop-shaped object.

**`EXPO_PUBLIC_*` env prefix is mandatory** (Expo strips others). Root `tsconfig.json` excludes `"mobile-app"` so the Vercel build never typechecks RN code — **don't remove that exclude**.

**Phase 5 handoff doc:** `mobile-app/PHASE_5_HANDOFF.md` — current state, Phase 5 follow-ups (POD photo persistence table, real line counts in route response, photo concurrency, image compression), Phase 6 plan (real maps + GPS-aware ETAs).

**Deferred:** rich per-job site contacts (`job_contacts` table on web), real maps + GPS ETAs (Phase 6), signature capture, barcode scan, background location tracking (App Store review).
