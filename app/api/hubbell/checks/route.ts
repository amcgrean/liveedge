...
export async function GET(req: NextRequest) {
  ...
  // baked into the query so we don't N+1.
  //
  //   doc_attached_so_ids: when doc_type IN ('po','wo') and a matching
  //     hubbell_documents row exists, aggregate attached SOs.
  //   inv_so_id: when doc_type = 'inv' and doc_number matches agility_so_header.so_id.
  //
  // The agility_so_header join uses ::int cast on doc_number; non-numeric
  // doc_numbers in 'inv' rows will return NULL and fall into 'unmatched'.
  ...
  const linesResultRaw = await db.execute(dsql`
    ...
    WHERE l.check_id = ANY(${checkIds}::uuid[])
