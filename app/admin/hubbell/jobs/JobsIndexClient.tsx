  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto">
      <div className="mb-4 flex items-center gap-2">
        ...
        <span className="ml-auto text-sm text-slate-400">Aggregated by customer + ship-to address</span>
      </div>
      ...
      {!loading && data && (
        <div className="overflow-x-auto rounded border border-slate-800">
          <table className="w-full text-sm">
            ...
            <tbody>
              {data.jobs.length === 0 && (
                <tr><td colSpan={6} className="px-3 py-6 text-center text-slate-500">No jobs yet</td></tr>
              )}
              {data.jobs.map((j) => (
                <tr key={`${j.cust_code}-${j.shipto_address_1}`} className="border-t border-slate-800 hover:bg-slate-900/50">
                  ...
                  <td className="px-3 py-2 text-slate-300">
                    {j.shipto_address_1 ?? '—'}
                    {j.shipto_city && <div className="text-xs text-slate-500">{j.shipto_city}, {j.shipto_state} {j.shipto_zip}</div>}
                  </td>
                  <td className="px-3 py-2 text-center">{j.doc_count}</td>
                  <td className="px-3 py-2 text-center">{j.so_count}</td>
                  <td className="px-3 py-2 text-right tabular-nums">
                    {j.hubbell_total ? `$${parseFloat(j.hubbell_total).toLocaleString()}` : '—'}
                  </td>
                  <td className="px-3 py-2 text-right">
                    {j.primary_so_id !== null && (
                      <Link href={`/admin/hubbell/jobs/${j.primary_so_id}`} className="text-cyan-400 hover:underline text-xs">
                        View →
                      </Link>
                    )}
                  </td>
