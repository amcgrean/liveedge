...
export default async function ProductMajorScorecard({
  params,
  searchParams,
}: {
  params: Promise<{ majorCode: string }>;
  searchParams: Promise<Record<string, string | string[]>>;
}) {
  const { majorCode: rawCode } = await params;
  const majorCode = decodeURIComponent(rawCode);
  ...
  const productFilter: ProductFilter = { level: 'major', majorCode };
  const drillParams: ProductDrillParams = {
    productFilter, branchIds, baseYear, compareYear, period, cutoffDate,
  };
  // aggregate params for the minor breakdown query (shared with branch/rep pages)
  const aggParams = { branchIds, baseYear, compareYear, period, cutoffDate };

  const [headerRes, kpisRes, threeYearRes, topCustomersRes, branchMixRes, saleTypesRes, minorsRes] =
    await Promise.allSettled([
      fetchProductHeader(productFilter),
      fetchProductKpis(drillParams, majorCode),
      ...
      fetchAggregateProductMinors(aggParams, majorCode),
    ]);
  ...
  const logFail = (name: string, r: PromiseSettledResult<unknown>) => {
    if (r.status === 'rejected') {
      failures.push(name);
      console.error(`[scorecard/product/major/${majorCode}] ${name}:`, r.reason);
    }
  };
  ...
  const minors = minorsRes.status === 'fulfilled' ? minorsRes.value : [];
  ...
  // Filter-state QS (used for breakdown row links so children inherit the same filters)
  const childQs = (() => {
    const sp = new URLSearchParams();
    sp.set('baseYear', String(baseYear));
    sp.set('compareYear', String(compareYear));
    sp.set('period', period);
    sp.set('cutoffDate', cutoffDate);
    branchIds.forEach((b) => sp.append('branch', b));
    return `?${sp.toString()}`;
  })();
    ...
    <div className="max-w-screen-xl mx-auto px-4 py-6 space-y-5">
      ...
      <div>
        <h1 className="text-2xl font-bold text-white">
          {title}
          <span className="font-normal text-base ml-2 text-slate-400">Product Group Scorecard</span>
        </h1>
      ...
      <AggregateFilterBar
        basePath={`/scorecard/product/major/${encodeURIComponent(majorCode)}`}
        baseYear={baseYear}
      ...
      <Section title="3-Year Comparison">
        <ComparisonTable entries={threeYear} exportFilename={`${majorCode}-3year`} />
      </Section>
      ...
      <Section title="Top Customers Buying This Product Group">
        <TopCustomersList rows={topCustomers} baseYear={baseYear} compareYear={compareYear} fromHint={`product-major:${majorCode}`} qs={childQs} />
      </Section>

      <Section title={`Product Minors in ${header?.majorName ?? majorCode}`}>
        <ProductDrillBreakdownTable
          rows={minors.map((m) => ({
            code: m.productMinorCode,
            label: m.productMinor,
            salesBase: m.salesBase,
            gpBase: m.gpBase,
            salesCompare: m.salesCompare,
            gpCompare: m.gpCompare,
          }))}
          level="minor"
          majorCode={majorCode}
          baseYear={baseYear}
          compareYear={compareYear}
          fromHint={`product-major:${majorCode}`}
          qs={childQs}
          exportFilename={`${majorCode}-minors-${baseYear}`}
        />
      </Section>
...
// ────────────────────────────────────────────────────────────────────────────
// Lightweight customer list table — links each row to /scorecard/[customerId]
// with `?from=product-major:<code>` so back navigation returns here.
// ────────────────────────────────────────────────────────────────────────────
