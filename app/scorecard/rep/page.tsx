        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-700 text-left">
              <th className="pb-2 text-slate-400 font-medium">Sales Rep</th>
              <th className="pb-2 text-slate-300 font-semibold text-right pr-4">{baseYear} Sales</th>
              <th className="pb-2 text-slate-300 font-semibold text-right pr-4">{compareYear} Sales</th>
              <th className="pb-2 text-slate-300 font-semibold text-right pr-4">{baseYear} GP</th>
              <th className="pb-2 text-slate-300 font-semibold text-right pr-4">GM%</th>
              <th className="pb-2 text-slate-300 font-semibold text-right">Customers</th>
            </tr>
          </thead>
          <tbody>
            {reps.length === 0 && (
              <tr>
                <td colSpan={6} className="py-12 text-center text-slate-500">
                  No rep data found — verify the sales_rep column exists in agility_so_header
                </td>
              </tr>
            )}
            {reps.map((r) => {
              const repUrl = `/scorecard/rep/${encodeURIComponent(r.repCode)}?baseYear=${baseYear}&compareYear=${compareYear}&period=${period}&cutoffDate=${cutoffDate}${branchIds.map((b) => `&branch=${b}`).join('')}`;
              return (
                <tr
                  key={r.repCode}
                ...
                >
                  <td className="py-2.5 pr-4">
                    <Link href={repUrl} className="flex items-center gap-1 group-hover:text-cyan-400 transition">
                  ...
                  <td className={`py-2.5 pr-4 text-right font-mono tabular-nums ${deltaClass(r.salesBase, r.salesCompare)}`}>
                    {fmt$(r.salesBase)}
                  </td>
                  <td className="py-2.5 pr-4 text-right font-mono tabular-nums text-slate-400">{fmt$(r.salesCompare)}</td>
                  <td className="py-2.5 pr-4 text-right font-mono tabular-nums text-slate-300">{fmt$(r.gpBase)}</td>
                  <td className="py-2.5 pr-4 text-right font-mono tabular-nums text-slate-300">{fmtPct(r.salesBase, r.gpBase)}</td>
                  <td className="py-2.5 text-right font-mono tabular-nums text-slate-400">{r.customerCount.toLocaleString()}</td>
                </tr>
              );
            })}
          </tbody>
